...
public class RestExceptionHandlerController {
	...
	public final ResponseEntity<ErrorResponse> exceptionConflictHandler(ConflictException ex) {
		log.info("exceptionConflictHandler - START - exceptionClass={}, exceptionMessage={}", ex.getClass().getSimpleName(), ex.getMessage());

		ErrorResponse errorResponse = new ErrorResponse();
		errorResponse.setCodiceDiStato(HttpStatus.NOT_ACCEPTABLE.value());
		
		List<String> messages = new ArrayList<>();
		messages.add(ex.getMessage());
		errorResponse.setMessaggi(messages);

		log.info("exceptionConflictHandler - END");
